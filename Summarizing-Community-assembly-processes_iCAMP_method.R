...
# # Lakeshore:
# iCAMP.IDs = read.csv("/Volumes/External/CHTC/Exp3/iCAMP/Lakeshore/iCAMP.process.CbMNTDiCBraya.csv", header=TRUE, check.names=FALSE, row.names=1)

# Arboretum/Gallistel:
iCAMP.IDs = read.csv("/Volumes/External/CHTC/Exp3/iCAMP/Arboretum/iCAMP.process.CbMNTDiCBraya.csv", header=TRUE, check.names=FALSE, row.names=1)
...
## Option to save
#write.csv(Proc.ID,"Derived_data/Community_Assembly_Process_IDs_iCAMP_Arboretum.csv")

# Read in data, if not continuing from above
Proc.ID = read.csv(file ="Derived_data/Community_Assembly_Process_IDs_iCAMP_Lakeshore.csv",
                           header=TRUE, stringsAsFactors=TRUE, row.names=1)

#### STACKED BAR GRAPH--Within Plot ####
Proc.ID
# Re-name variables and put them in order
Proc.ID$com1.trt = recode_factor(Proc.ID$com1.trt, "NoWorm" = "Control", "LowWorm" = "Low","HighWorm" = "High")
Proc.ID$com1.trt = ordered(Proc.ID$com1.trt, levels = c("Control", "Low", "High"))
...
p.2 = ggplot(subset(Proc.ID, sameplot=="same"), aes(x=com1.trt, y=Proportion)) +
  ...
  scale_x_discrete(labels=c("NoWorm"="Control", "LowWorm"="Low Amynthas", "HighWorm"="High Amynthas")) +
  labs(x = "Within-plot sample comparisons",
       y = "Relative influence, Gallistel", fill = "Community assembly process") +
...
ggarrange(Arb.iCAMP.plot, Arb.iCAMP.trt,
  Lak.iCAMP.plot, Lak.iCAMP.trt,
  labels = c("A", "B", "C", "D"),
  ncol = 2, nrow = 2,
  common.legend = TRUE, legend = "bottom")

ggsave("Figures/Community_Assembly.iCAMP_WORM2.tiff", width=9.5, height=7.5, units = "in", device='tiff', dpi=400)



### Statistics-- identify significant changes to homogenizing dispersal and homogeneous selection
# must repeat for both Arboretum and Lakeshore data
head(same_plot)
