...
ps = readRDS("ps.Exp3")
ps.site = subset_samples(ps,sample_data(ps)$site == "Arb" |
                           sample_data(ps)$site == "Lak")
ps.site
...
#### Differential abundance, Amynthas pressure treatments ####

# FIRST! Create empty data frame to hold all results
df = data.frame()

# Subset for one comparison: Control vs Low (or high) Amynthas pressure within a fraction (and within site)
# Loop to run through each site x trt x fraction combination
for (i in c("Arb", "Lak")){
  ...
  for (l in c("fresh", "freem", "occm")){
    ...
    for (m in c("LowWorm", "HighWorm")){
      ps.subset = prune_samples(sample_data(ps.subset)$trt == "NoWorm" | sample_data(ps.subset)$trt == m, ps.subset)
    ...
    ps.sub = prune_samples(sample_data(ps.sub)$trt == m, ps.sub)
...
#write.csv(df,"Derived_data/Differential_Abundance_Amynthas_trt_differences_0.00001_0.001.csv", row.names = FALSE)
# 
df <- read.csv(file = 'Derived_data/Differential_Abundance_Amynthas_trt_differences_0.00001_0.001.csv')
head(df)
...
# Write to .csv
write.csv(df2,"Derived_data/Amynthas_enriched_taxa_with_sequences.csv", row.names = FALSE)

write.csv(df2,"Derived_data/Amynthas_ALL_enriched_taxa_>0_withRare_with_sequences.csv", row.names = FALSE)
...
Big.pos$trt = recode_factor(Big.pos$trt,
                                 "LowWorm" = "Low Amynthas pressure",
                                 "HighWorm" = "High Amynthas pressure")
Big.pos$site = recode_factor(Big.pos$site,
                                  Arb = "Gallistel",
                                  Lak = "Lakeshore")
AmynthasX = expression(paste("Coefficient of differential abundance for taxa enriched under ", italic("Amynthas"), " pressure"))

p = ggplot(Big.pos,aes(x=Estimate, color=Phylum, shape = fraction, y=reorder(OTU, Estimate)))
...
p = p + xlab(AmynthasX) + ylab("")
p = p + scale_y_discrete(breaks=Big.pos$OTU,labels=Big.pos$Name)
p = p + facet_grid(vars(site), vars(trt), scales="free_y", space="free_y")
p = p + scale_shape_manual(values = c(1, 17, 14), name = "Soil fraction")
#p = p + geom_hline(xintercept=0)
p = p + theme(legend.position = "right", legend.text=element_text(size=rel(0.75)), 
              legend.title = element_text(size=rel(0.85)), legend.spacing.y = unit(0.01, 'cm'))
p = p + guides(color = guide_legend(byrow = TRUE))
p #720 x 650?

#ggsave("Figures/DiffAbund_pos_Amynthas.tiff", width=7.5, height=7.5, units = "in", device='tiff', dpi=400)
...
Big.neg$fraction = recode_factor(Big.neg$fraction,
                                 ...
                                 "freem" = "Free microaggregate",
                                 "occm" = "Occluded microaggregate")
Big.neg$trt = recode_factor(Big.neg$trt,
                              "LowWorm" = "Low Amynthas pressure",
                              "HighWorm" = "High Amynthas pressure")
Big.neg$site = recode_factor(Big.neg$site,
                             Arb = "Gallistel",
                             Lak = "Lakeshore")
AmynthasX2 = expression(paste("Coefficient of differential abundance for taxa depleted under ", italic("Amynthas"), " pressure"))


p = ggplot(Big.neg, aes(x=-Estimate, color=Phylum, y= reorder(OTU, -Estimate), shape = fraction))
p = p + theme_bw() + theme(#panel.grid.major = element_blank(), 
                                              panel.grid.minor = element_blank(), 
                                              strip.background = element_rect(fill="white"))
p = p + geom_point(size=2)
...
#ggsave("Figures/DiffAbund_neg.Amynthas.tiff", width=6.75, height=5, units = "in", device='tiff', dpi=400)
